...
with tf.Graph().as_default():
    with tf.name_scope('input'):
        ftr_in = tf.compat.v1.placeholder(dtype=tf.float32, shape=(batch_size, nb_nodes, ft_size))
        if sparse:
            #bias_idx = tf.compat.v1.placeholder(tf.int64)
            #bias_val = tf.compat.v1.placeholder(tf.float32)
            #bias_shape = tf.compat.v1.placeholder(tf.int64)
        ...
        lbl_in = tf.compat.v1.placeholder(dtype=tf.int32, shape=(batch_size, nb_nodes, nb_classes))
